...
def main():
    ...
    #os.makedirs(args.accelerator_coop_ckpts_dir, exist_ok=True)
    ...
    for key, value in block_names.items():
        setattr(slowAttentionBlocks, key, AttentionBlock(2048, value))
        setattr(fastAttentionBlocks, key, AttentionBlock(256, value))
    ...
    train_dataloader = read_hdf5_file('lsvq_train_latent.h5')

    # Similarly generate the latent for any test dataset
    # Nevertheless we have provided the code to generate latent for KoNViD-1k
    # dataset (konvid.py). Usew this code to generate the test latents.
    test_dataloader = read_hdf5_file('konvid_test_latent.h5')
    ...
    for batches in test_dataloader.values():
        length_of_test_dataset += len(batches['mos_scores'])
    ...
    slowAttentionBlocks.load_state_dict(torch.load(os.path.join(os.path.join(args.slowAttn_ckpts_dir, best_val_weights_path), 'slowAttn.pth')))
    fastAttentionBlocks.load_state_dict(torch.load(os.path.join(os.path.join(args.fastAttn_ckpts_dir, best_val_weights_path), 'fastAttn.pth')))
    slowAttentionBlocks.eval()
    fastAttentionBlocks.eval()
    logger.info(f'Loading the SlowAttention Blocks from {os.path.join(args.slowAttn_ckpts_dir, best_val_weights_path)}')
    logger.info(f'Loading the fastAttention Blocks from {os.path.join(args.fastAttn_ckpts_dir, best_val_weights_path)}')

    linearBlocks.load_state_dict(torch.load(os.path.join(os.path.join(args.linBlock_ckpts_dir, best_val_weights_path), 'linearBlocks.pth')))
    logger.info(f'Loading the Linear Blocks from {os.path.join(args.linBlock_ckpts_dir, best_val_weights_path)}')
    ...
    with torch.no_grad():
        ...
        slowAttentionBlocks = accelerator.unwrap_model(slowAttentionBlocks)
        fastAttentionBlocks = accelerator.unwrap_model(fastAttentionBlocks)
        ...
        num_test_samples = 0
        for batch_idx, _ in  tqdm(test_dataloader.items(), total=len(test_dataloader)):
            
            batch = test_dataloader[batch_idx]
            scores = []
            
            videos = torch.from_numpy(batch['datas']).to(accelerator.device)
            unidentified_indices = []

            for video_idx in range(0, videos.shape[0]):
                video = videos[video_idx]
                filename = batch['filenames'][video_idx]
                
                score = compute_score_genzvqa(args, video, filename, pipeline_img2img,
                                                slowAttentionBlocks, fastAttentionBlocks, 
                                                scale_range, linearBlocks, text_model = coop_Model,                                                 
                                                fixed_prompt_embeds=None)
                if score is not None:
                    scores.append(score)
                else:
                    scores.append(torch.tensor(0).to(accelerator.device))
                    unidentified_indices.append(video_idx)
            ...
            if GT_MOS_Scores.dim() < 1:
                GT_MOS_Scores = GT_MOS_Scores.unsqueeze(0)
            ...
            for filename_idx in range(len(GT_MOS_Scores)):  
                if filename_idx not in unidentified_indices:
                    filename = batch['filenames'][filename_idx]
        ...
        predicted_quality = np.array(predicted_quality)
        actual_quality = np.array(actual_quality)
        
        test_loss_coop = test_loss_coop / num_test_samples
        fit = np.poly1d(np.polyfit(predicted_quality, actual_quality, deg = 3))
        predicted_quality = fit(predicted_quality)
    
    test_srocc_score = (stats.spearmanr(actual_quality, predicted_quality)).statistic
    test_plcc_score = (stats.pearsonr(actual_quality, predicted_quality)).statistic
